 ...
 * <p>
 * Item factories that need an actual {@code EntityType<T>} instance (BoatItem, ForgeSpawnEggItem's
 * backing supplier) must never call {@code .get()} outside a deferred supplier - see BlockRegistry.java's
 * javadoc on the same trap. Every boat/spawn-egg item factory below is itself a lambda, so
 * {@code EntityTypeRegistry.X.get()} inside it is safe (only evaluated when Forge invokes the
 * stored Item supplier during RegisterEvent&lt;Item&gt;, well after RegisterEvent&lt;EntityType&gt;).
 * <p>
 * {@code FabricDefaultAttributeRegistry.register(...)} has no Forge equivalent; the Forge event is
 * {@link EntityAttributeCreationEvent}, fired on the mod bus. The attribute-supplying static methods
 * live on this file's own entity classes (TargetDummy.createTargetDummyAttributes() etc.), which
 * this file already imports - so {@link #registerAttributes} is exposed here as a plain static
 * method for whichever package's @Mod.EventBusSubscriber ends up calling it from a
 * {@code @SubscribeEvent public static void on(EntityAttributeCreationEvent event)} handler
 * (not this file's own event subscription, since ModBusEvents.java lives in a different package).
 */
public class EntityTypeRegistry {
    ...
    // matching the same discipline as everywhere else in this pass.
    ...
    public static List<EntityType<? extends AbstractBoat>> vanillaBoats() {
        return List.of(EntityTypes.ACACIA_BOAT, EntityTypes.BAMBOO_RAFT, EntityTypes.BIRCH_BOAT, EntityTypes.CHERRY_BOAT, EntityTypes.DARK_OAK_BOAT, EntityTypes.JUNGLE_BOAT, EntityTypes.MANGROVE_BOAT, EntityTypes.OAK_BOAT, EntityTypes.PALE_OAK_BOAT, EntityTypes.SPRUCE_BOAT, BAOBAB_BOAT.get());
    }
    ...
    /**
     * FabricDefaultAttributeRegistry -> Forge's EntityAttributeCreationEvent. Call this from a
     * {@code @SubscribeEvent public static void onEntityAttributeCreation(EntityAttributeCreationEvent
     * event)} handler on the mod bus (e.g. ModBusEvents.java).
     */
    public static void registerAttributes(EntityAttributeCreationEvent event) {
        event.put(TARGET_DUMMY.get(), TargetDummy.createTargetDummyAttributes().build());
        event.put(WILDFIRE.get(), WildfireEntity.createWildfireAttributes().build());
        event.put(TERMITE.get(), Termite.createAttributes());
        event.put(MOOBLOOM.get(), Moobloom.createAttributes());
        event.put(SUSPICIOUS_SPIDER.get(), SuspiciousSpider.createSuspiciousSpiderAttributes());
        event.put(DERELICT.get(), Zombie.createAttributes());
        event.put(RIME.get(), Zombie.createAttributes());
        event.put(DRENCHED.get(), Drenched.createDrenchedAttributes());
    }
}
