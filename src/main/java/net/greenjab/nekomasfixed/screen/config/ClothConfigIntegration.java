 ...
 * ModList.get().isLoaded("cloth_config")} check, so {@code ConfigTrial::createConfigScreen} (and
 * therefore Cloth's own classes, transitively) is only ever resolved when that branch actually
...
public final class ClothConfigIntegration {
    ...
    // referenced - and therefore never resolved/classloaded - unless isLoaded("cloth_config") was true.
