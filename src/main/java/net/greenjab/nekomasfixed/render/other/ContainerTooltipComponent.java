 ...
 * GuiGraphicsExtractor)} becomes 1.20.1's real {@code renderImage(Font, x, y, GuiGraphics)}; the one
 * {@code blitSprite} call goes through {@link LegacySpriteBlit} (see its javadoc). This DOES restore
 * the container-contents tooltip grid (slot backgrounds + item icons + count/durability
...
public class ContainerTooltipComponent implements ClientTooltipComponent {
    private static final ResourceLocation BUNDLE_SLOT_BACKGROUND_TEXTURE = new ResourceLocation("minecraft", "container/bundle/slot_background");
    ...
    private void drawItem(int x, int y, List<ItemStack> stacks, int index, Font font, GuiGraphics guiGraphics) {
        ...
        LegacySpriteBlit.blitSprite(guiGraphics, BUNDLE_SLOT_BACKGROUND_TEXTURE, x, y, 24, 24);
